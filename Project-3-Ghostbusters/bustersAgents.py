...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #finding most likely postion...identifying target ghost
        for distributions in livingGhostPositionDistributions:
            position = distributions.argMax()
            distance = self.distancer.getDistance(pacmanPosition, position)
            if targetDistance > distance:
                targetDistance = distance
                targetPosition = position
        #find which move gets us closest
        newAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            newDist = self.distancer.getDistance(successorPosition, targetPosition)
